...
Subcommands:
  run <topic>      - Execute research on a topic
  validate <card>  - Validate a research card JSON
  list             - List existing research cards
  dedup <card>     - Check deduplication for a card
  index            - Manage FAISS vector index
  seed-gen <card>  - Generate Story Seed from research card
  seed-list        - List available Story Seeds
"""
...
def _validate_topic(topic: str) -> Optional[str]:
    """Validate topic string. Returns error message if invalid, None if valid."""
...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    # Collapse canonical_affinity to canonical_core
    canonical_core = None
    try:
        ...
        is_valid, err = validate_canonical_core(canonical_core)
        if is_valid:
            logger.info(f"[ResearchExec] Canonical core: {canonical_core}")
        else:
            logger.warning(f"[ResearchExec] Invalid canonical_core: {err}")
    except Exception as e:
        logger.warning(f"[ResearchExec] Failed to collapse canonical: {e}")

    # Build preliminary card data for dedup check
    preliminary_card = {
        "card_id": card_id,
        "output": output,
    ...
    # Print summary (parsed by research_service.py)
        ...
        if args.rebuild:
            ...
            for card_path in card_paths:
                try:
                    with open(card_path, "r", encoding="utf-8") as f:
                        card_data = json.load(f)
                    cid = card_data.get("card_id", card_path.stem)
                    if add_card_to_index(card_data, cid, index=index, save=False):
                        added += 1
                        print(f"  Indexed: {cid}")
                except Exception as e:
            ...
            if add_card_to_index(card_data, cid, index=index):
                print(f"Indexed: {cid} (total: {index.size})")
            else:
                print(f"Failed to index: {cid}")
                return EXIT_INVALID_INPUT
        else:
...
def create_parser() -> argparse.ArgumentParser:
    ...
    run_parser.add_argument("topic", help="Research topic to analyze")
    run_parser.add_argument(
        "-m", "--model",
        default=DEFAULT_MODEL,
        help=f"Model to use. Default: {DEFAULT_MODEL}. "
             "Formats: 'qwen3:30b' (Ollama), 'gemini' (Gemini API), "
    ...
    validate_parser.add_argument("card", help="Path to research card JSON file")

    # dedup command
    dedup_parser = subparsers.add_parser("dedup", help="Check deduplication for a research card")
    dedup_parser.add_argument("card", help="Path to research card JSON file")
    ...
    index_parser.add_argument("-c", "--card", help="Path to research card to index")
    index_parser.add_argument("--rebuild", action="store_true", help="Rebuild entire index from all cards")

    # seed-gen command
    seed_gen_parser = subparsers.add_parser("seed-gen", help="Generate Story Seed from research card")
    seed_gen_parser.add_argument("card", help="Path to research card JSON file")
